...
### Amendements économie (user, 2026-08-03)
| # | Décision | Statut |
|---|---|---|
| D72 | **FREE = DÉMO HORS-CHAÎNE SÉPARÉE** (amende A0.7bis) : rejoindre en FREE = partie **avec des BOTS uniquement**, complètement off-chain, façon tutorial. **Aucune valeur lamport à extraire, et on n'y croise JAMAIS un joueur ayant misé du SOL.** Ce n'est plus un « tier » du serveur réel — deux mondes étanches dès le MVP (le bouton FREE du serveur payant doit disparaître ou router vers la démo) | ✅ acté |
| D73 | **MATÉRIALISATION IMMÉDIATE des pellets** (amende D71) : la part pellet-fund de **CHAQUE mise** est convertie **en pellets au moment du join** (valeur en fractions de lamports), pas un pool diffus qui paie des respawns au fil de l'eau. **1 seul joueur connecté = uniquement SES pellets sur la map.** Map presque vide à faible population = **VOULU** (la densité de bouffe suit les dépôts). Corollaire : **plus AUCUN respawn ambiant gratuit** — la map ne se regarnit que par nouveaux joins, cadavres (70/30) et traînées de boost | ✅ acté |
| D74 | **SOLVABILITÉ : les lamports des pellets vivent AU VAULT** (2026-08-03). Les extractions ne payent que depuis le Vault (D55) → un pellet adossé au PDA FoodReserve rendrait le Vault insolvable au dernier extracteur. Fix sans toucher au programme : **`initialize_round` avec `reserve_bps = 0`** (94,5 % du net au Vault), la part pellet devient **comptabilité serveur**. Le PDA FoodReserve garde le sweep D50 + injections marketing. Le serveur REFUSE au boot un round dont reserve_bps ≠ 0 (double-comptage sinon) | ✅ acté |
| D75 | **Pellet cut = 10 % de la mise** (choix user contre la reco 2 % — curseur `PELLET_CUT_BPS`, réglable ENTRE rounds). Split effectif : **5,5 % rake / 10 % pellets / 84,5 % spawn**. ⚠ Réitéré (D71) : 10 % = revenu sans-combat maximal pour un bot rentier — à MESURER au red-team A4.6, re-tuning attendu | ✅ acté |
| D76 | **Démo FREE construite DÈS A3.2ter** (pas reportée) : room séparée avec bots serveur, hors-chaîne, **sans wallet requis** (SIWS protège de l'argent, une démo n'en a pas — proposition Claude, veto user possible). Le 70/30 in-game reste inchangé partout | ✅ acté |
